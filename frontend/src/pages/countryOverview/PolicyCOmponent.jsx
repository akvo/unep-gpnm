...
const FrancePolicyComponent = ({ cashReturn, subsidies,education,labeling,bansOnPlastic, limitsPlastic,plasticLeakage, country }) => (
  <Card
    style={{
      width: '65%',
      borderRadius: '12px',
...
const PolicyComponent = () => {
  ...
  useEffect(() => {
    ...
    const bansOnPlasticLayer = layers.find(
      (layer) =>
        layer.attributes.arcgislayerId === 'Policies_with_bans_on_plastic_6_10_24_WFL1'
    )

    const limitsOnPlasticLayer = layers.find(
      (layer) =>
        layer.attributes.arcgislayerId === 'Policies_with_limits_on_plastic_6_10_24_WFL1'
    )
    const plasticLeakageLayer = layers.find(
      (layer) =>
        layer.attributes.arcgislayerId === 'Regulations_on_new_practices_to_reduce_plastic_leakage_WFL1'
    )
